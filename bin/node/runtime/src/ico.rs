...
// 	/// todo 增加或是减少募集的金额
// 	fn change_specific_amount(asset_id: T::AssetId, symbol_enum: AddressEnum<T::AccountId>, amount: Balance, usdt_amount: USDT, is_sub: bool) -> DispatchResult {
// 		// asset_id 资产id
// 		// symbol_enum 参与ico的玩家的代币地址
// 		// amount 代币的数量
// 		// usdt_amount usdt的个数
// 		// is_sub 是否是减掉
//
// 		match symbol_enum {
// 			// 先解决dico
// 			AddressEnum::<T::AccountId>::Dico(player_address) => {
// 				let (project_name, symbol) = <SymbolOf<T>>::get(asset_id.clone()).ok_or(Error::<T>::SymbolNotExists)?;
// 				let project = <Projects<T>>::get(project_name, symbol).ok_or(Error::<T>::ProjectNotExists)?;
// 				// 获取项目方给的所有地址
// 				let address = project.1.public_keys;
...
// 				// 如果dico地址存在 才会进行下面的步骤
// 				if dico_address_opt.is_some() {
// 					let dico_address = dico_address_opt.unwrap();
// 					let specific_raise_amount_opt = <SpecificRaiseAmount<T>>::get(asset_id.clone());
//
// 					// 如果信息存在  说明不是第一次添加
// 					if specific_raise_amount_opt.is_some() {
// 						{
// 						let mut specific_raise_amount = specific_raise_amount_opt.clone().unwrap();
// 						let project_manager_get = specific_raise_amount.clone().project_manager_get;
// 						let manager_dico_address_opt = project_manager_get.dico;
//
// 						// 如果已经存在 说明之前有添加过
// 						if manager_dico_address_opt.is_some() {
// 							let mut manager_dico_address = manager_dico_address_opt.clone().unwrap();
// 							manager_dico_address.1 += amount;
// 							manager_dico_address.2 += usdt_amount;
// 							specific_raise_amount.project_manager_get.dico = Some(manager_dico_address);
//
// 						}
// 						// 之前没有添加过
// 						else {
// 							let manager_dico_address = (AddressEnum::<T::AccountId>::Dico(dico_address), amount, usdt_amount);
// 							specific_raise_amount.project_manager_get.dico = Some(manager_dico_address);
//
// 						}
// 						}
...
// 						{
// 						let mut specific_raise_amount = specific_raise_amount_opt.clone().unwrap();
// 						let others_send = specific_raise_amount.clone().others_send;
// 						let others_opt = project_manager_get.get(player_address);
// 						// 如果之前有
// 						if others_opt.is_some() {
//
// 						}
//
// 						// 如果已经存在 说明之前有添加过
