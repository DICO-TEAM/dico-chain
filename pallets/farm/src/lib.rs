...
impl PoolInfo {
	pub fn new(currency_id: AssetId, alloc_point: u128, last_reward_block: BlockNumber) -> Self {
		Self {
			currency_id,
			alloc_point,
			last_reward_block,
...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn withdraw_lp(origin: OriginFor<T>, pool_id: T::PoolId, amount: Balance) -> DispatchResultWithPostInfo {
			...
			if pending_amount > Balance::zero() {
				T::Currency::transfer(native_asset, &module_account_id, &who, pending_amount)?;
			}
...
impl<T: Config> Pallet<T> {
	...
	fn reward(block_number: BlockNumber) -> sp_std::result::Result<Balance, ArithmeticError> {
		let phase: U256 = Self::phase(block_number)?;
	...
	fn get_dico_block_reward(last_reward_block: BlockNumber) -> sp_std::result::Result<Balance, ArithmeticError> {
		...
		if block_number <= last_reward_block {
			return to_balance!(0);
		}

		let mut last_reward_block_hp = to_u256!(last_reward_block);
	...
	fn calc_participant_reward(
		...
		let acc_dico_per_share = if block_number <= pool.last_reward_block || pool.alloc_point == 0u128 {
