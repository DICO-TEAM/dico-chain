...
use super::*;
pub use crate::mock::{
	Currency, Event as TestEvent, ExtBuilder, Farm, Origin, System, Test, ALICE, BOB, DEFAULT_ASSET_AMOUNT, DICO, DOT,
	PDOTUSDT, USDT,
};
...
fn set_halving_period_should_work() {
	new_test_ext().execute_with(|| {
		assert_ok!(Farm::set_halving_period(Origin::signed(ALICE), 1000));
...
fn set_dico_per_block_should_work() {
	new_test_ext().execute_with(|| {
		assert_ok!(Farm::set_dico_per_block(Origin::signed(ALICE), 10000));
...
fn set_start_block_should_work() {
	new_test_ext().execute_with(|| {
		assert_ok!(Farm::set_start_block(Origin::signed(ALICE), 10000));
...
fn create_pool_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Farm::create_pool(Origin::signed(ALICE), PDOTUSDT, alloc_point));
...
fn update_pool_alloc_point_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Farm::update_pool_alloc_point(Origin::signed(ALICE), 0, 10000u128));
...
fn deposit_lp_should_work() {
	new_test_ext().execute_with(|| {
		assert_ok!(Farm::set_halving_period(Origin::signed(ALICE), 5000));

		assert_ok!(Farm::set_dico_per_block(Origin::signed(ALICE), 100_000_000_000_000));

		assert_ok!(Farm::set_start_block(Origin::signed(ALICE), 1000));

		let liquidity_id: AssetId = DOT;
		let alloc_point = 1000u128;

		assert_ok!(Farm::create_pool(Origin::signed(ALICE), liquidity_id, alloc_point));
		...
		assert_ok!(Farm::deposit_lp(Origin::signed(ALICE), 0, 100_000_000_000_000));
		...
		assert_eq!(
			Currency::free_balance(liquidity_id, &module_id_account),
			100_000_000_000_000
		);
		assert_eq!(
			Currency::free_balance(liquidity_id, &ALICE),
		...
		assert_ok!(Farm::deposit_lp(Origin::signed(BOB), 0, 200_000_000_000_000));
...
fn deposit_lp_should_work2() {
	new_test_ext().execute_with(|| {
		...
		System::set_block_number(16001);
		assert_ok!(Farm::update_pool_alloc_point(Origin::signed(ALICE), 0, 0));
		...
		assert_eq!(
			alice_reward,
			100_000_000_000_000u128 * 8750125000000000u128 / 1e12 as u128 - 0u128
		);
...
fn withdraw_lp_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Farm::withdraw_lp(Origin::signed(ALICE), 0, 0));
		...
		assert_eq!(
			alice_reward,
			100_000_000_000_000u128 * 8812625000000000u128 / 1e12 as u128 - 875012500000000000u128
		);
...
fn withdraw_lp_should_work2() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Farm::withdraw_lp(Origin::signed(ALICE), 0, 50_000_000_000_000));
		...
		assert_eq!(
			Currency::free_balance(liquidity_id, &ALICE),
			DEFAULT_ASSET_AMOUNT - 50_000_000_000_000
		);
		assert_eq!(Currency::free_balance(0, &module_id_account), 0);
	});
}
