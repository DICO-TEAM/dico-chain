...
fn set_propose<T: Config>() -> (T::Hash, u32, u32) {
	...
	let proposal: T::Proposal = SystemCall::<T>::remark { remark: vec![1; 100] }.into();
...
benchmarks! {
	propose {
		...
		let ico_index = 1;
		T::IcoHandler::set_ico_for_bench(currency_id, ico_index, caller.clone(), get_alice::<T>(), get_bob::<T>());
	}:_(RawOrigin::Signed(get_alice::<T>()), currency_id, 1, Percent::from_percent(30u8), Box::new(proposal), vec![1;100], 100)
