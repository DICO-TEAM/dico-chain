...
fn insert_feed_account_should_work() {
	new_test_ext().execute_with(|| {
		assert_noop!(PriceDao::insert_feed_account(Origin::signed(GAVIN),vec![ALICE,BOB]), BadOrigin);

		assert_ok!(PriceDao::insert_feed_account(Origin::signed(ALICE),vec![ALICE,BOB,DAVE]));
		assert_eq!(crate::DepositBalance::<Test>::get(&BOB),
				   Some(crate::DepositBalanceInfo{amount: 50_000_000_000_000_000,expiration: 0}));
		...
		assert_eq!(Balances::free_balance(&BOB), 2000_000_000_000_000_000-50_000_000_000_000_000);
		assert_eq!(Balances::reserved_balance(&BOB), 50_000_000_000_000_000);
	});
}

#[test]
fn del_feed_account_should_work() {  // slash
	new_test_ext().execute_with(|| {
		assert_eq!(<Balances as Currency<AccountId>>::total_balance(&BOB), 2000_000_000_000_000_000);
...
fn exit_feed_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(Balances::reserved_balance(&BOB), 50_000_000_000_000_000);
		assert_eq!(crate::DepositBalance::<Test>::get(&BOB),
				   Some(crate::DepositBalanceInfo{amount: 50_000_000_000_000_000,expiration: 11}));

		System::set_block_number(11);
		assert_noop!(PriceDao::exit_feed(Origin::signed(BOB)), Error::<Test>::ExpirationNotEmpty);
		assert_eq!(Balances::reserved_balance(&BOB), 50_000_000_000_000_000);
