...
impl<T: Config> Module<T> {
	...
	fn is_exclude_area(who: &T::AccountId, currency_id: &AssetId, index: u32) -> result::Result<bool, DispatchError> {
		let ico = <Ico<T>>::get(currency_id, index).ok_or(Error::<T>::IcoNotExists)?;
	...
	fn unreserve_initiator_remain_token(
		...
		let index = ico.index.unwrap();
		let (project_total_amount, _, _) = Self::get_total_and_released_amount(ico.currency_id, index, &ico.initiator);
	...
	fn do_release(
		...
		if Self::is_ico_expire(&ico)
			&& (IsUnservePledge::contains_key(currency_id, index) && IsUnservePledge::get(currency_id, index))
		{
			if is_do {
	...
	pub fn can_join_amount(
		...
		let user_remain_exchange_amount = Self::u256_convert_to_balance(
			Self::balance_convert_to_u256(user_remain_usdt) * Self::balance_convert_to_u256(decimals_convert_amount)
...
impl<T: Config> IcoHandler<AssetId, MultiBalanceOf<T>, T::AccountId, DispatchError, T::BlockNumber> for Module<T> {
