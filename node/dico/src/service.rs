...
pub struct KICORuntimeExecutor;

impl sc_executor::NativeExecutionDispatch for KICORuntimeExecutor {
	type ExtendHostFunctions = frame_benchmarking::benchmarking::HostFunctions;

	fn dispatch(method: &str, data: &[u8]) -> Option<Vec<u8>> {
		kico_runtime::api::dispatch(method, data)
	}

	fn native_version() -> sc_executor::NativeVersion {
		kico_runtime::native_version()
	}
}

pub struct TICORuntimeExecutor;

impl sc_executor::NativeExecutionDispatch for TICORuntimeExecutor {
	type ExtendHostFunctions = frame_benchmarking::benchmarking::HostFunctions;

	fn dispatch(method: &str, data: &[u8]) -> Option<Vec<u8>> {
		tico_runtime::api::dispatch(method, data)
	}

	fn native_version() -> sc_executor::NativeVersion {
		tico_runtime::native_version()
	}
}

pub trait IdentifyVariant {
	fn is_tico(&self) -> bool;

	fn is_kico(&self) -> bool;
}

impl IdentifyVariant for Box<dyn sc_service::ChainSpec> {
	fn is_tico(&self) -> bool {
		self.id().to_lowercase().starts_with("tico")
	}

	fn is_kico(&self) -> bool {
		self.id().to_lowercase().starts_with("kico")
	}
}
