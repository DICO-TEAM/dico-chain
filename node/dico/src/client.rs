...
pub trait RuntimeApiCollection:
	...
	+ pallet_ico_rpc_runtime_api::IcoAmountApi<Block, AccountId, AssetId, Index, Balance>
where
...
///
/// As there exist multiple chains inside DICO, like DICO itself, KICO etc,
...
/// A handle to a DICO client instance.
///
/// The DICO service supports multiple different runtimes. As each
/// runtime has a specialized client, we need to hide them behind a trait. This is this trait.
///
...
macro_rules! with_client {
	{
		...
		match $self {
			Self::TICO($client) => { $( $code )* },
            Self::KICO($client) => { $( $code )* }
		}
	}
}
...
impl sc_client_api::BlockBackend<Block> for Client {
	fn block_body(&self, id: &BlockId) -> sp_blockchain::Result<Option<Vec<<Block as BlockT>::Extrinsic>>> {
		with_client! {
			self,
			client,
			{
...
impl sc_client_api::StorageProvider<Block, crate::service::FullBackend> for Client {
	fn storage(&self, id: &BlockId, key: &StorageKey) -> sp_blockchain::Result<Option<StorageData>> {
		with_client! {
			self,
			client,
			{
	...
	fn storage_keys(&self, id: &BlockId, key_prefix: &StorageKey) -> sp_blockchain::Result<Vec<StorageKey>> {
		with_client! {
			self,
			client,
			{
	...
	fn storage_hash(&self, id: &BlockId, key: &StorageKey) -> sp_blockchain::Result<Option<<Block as BlockT>::Hash>> {
		with_client! {
			self,
			client,
			{
